...
<%
    ...
    int sortid		=	(Integer)request.getAttribute("sortid");  //一共多少记录
	...
	function deleteAll() {
		...
		for (var i = 0; i < len; i++) {
 		...
 		if(deletelist==""){
 			...
 			if(confirm("确定要删除？")){
 				window.location.href="<%=basePath%>goods/goods_doDeleteAll.action?deletelist="+deletelist+"&currentPage=<%=currentPage%>&sortid=<%=sortid%>";
			}
		}
	}
</script>
	...
	<form action="<%=basePath%>goods/goods_doFind.action" name="goodslistForm" id="goodslistForm" method="post">
		<div class="place">
			...
			<ul class="placeul">
				...
				<li1><a style="color: blue;" <%=basePath%>sort/sort_doFind.action">分类列表</a></li1>
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<tr>
						...
						<a href="<%=basePath%>goods/goods_doDelete.action?goods.goodsId=<%=goods.getGoodsId()%>&currentPage=<%=currentPage%>&sortid=<%=sortid%>" onClick="return checkDelete()" style="cursor:hand;" class="tablelink1">删除</a>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					...
					%>
					<li class="paginItem"><a href="<%=basePath%>goods/goods_doFind.action?currentPage=<%=firstPage-PAGE_SIZE%>&sortid=<%=sortid%>&keyword=<%=keyword%>">
					...
					<li class="paginItem current"><a href="<%=basePath%>goods/goods_doFind.action?currentPage=<%=i%>&sortid=<%=sortid%>&keyword=<%=keyword%>"><%=i%></a></li>

					<%
						continue;}
					%>

					<li class="paginItem"><a href="<%=basePath%>goods/goods_doFind.action?currentPage=<%=i%>&sortid=<%=sortid%>&keyword=<%=keyword%>"><%=i%></a></li>
