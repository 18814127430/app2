...
<body>
	<form action="<%=basePath%>sort/sort_doUpdate.action" method="post" onsubmit="return checkForm();" enctype="multipart/form-data" name="form1">
		<div class="place">
			<span>位置：</span>
			<ul class="placeul">
			<li1><a href="<%=basePath%>mainindex.jsp">首页</a></li1>
				...
				<li1><a href="<%=basePath%>goods/goods_doFind.action?sortid=<%=sort.getSortId()%>">商品列表</a></li1>
				<li1><a  style="color:blue;" href="<%=basePath%>sort/sort_doEdit.action?sort.sortId=<%=sort.getSortId()%>">分类修改</a></li1>
		...
		<div class="MID">
			<div class="MID1">
				...
				<div class="ibox"></div>
				<ul class="forminfo">
					<li><input type="hidden" name="sort.sortId" id="sort.sortId" value='<%=sort.getSortId()%>' /></li>
					<li><input type="hidden" name="sort.sortClass" id="sort.sortClass" value='<%=sort.getSortClass()%>' /></li>
					<li><label>类别名<b>*</b></label> <input type="text" class="dfinput" name="sort.sortName" id="sort.sortName" value='<%=sort.getSortName()%>' style="width:320px;" /></li>
			...
			<div class="MID2">
				<ul class="forminfo1">
					...
					<li>
						<div id="localImage">
							<img src="<%=sort.getSortImgPath()%>" id="preview" width=150 height=160 style="diplay:none" />
						</div>
					</li>
					<div class="ibox"></div>

					<li><input type="hidden" id="url1" name="sort.sortImgPath" value='<%=sort.getSortImgPath()%>'></li>
					<li><p>
