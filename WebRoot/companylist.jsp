	...
	function edit() {
		...
		for (var i = 0; i < len; i++) {
 		...
 		if(flag==0){
			...
			if(confirm("确定要修改？")){
 				window.location.href="<%=basePath%>company/company_doEdit.action?company.companyId="+editid;
			}
		}
 		else{
	...
	function deleteAll() {
		...
		for (var i = 0; i < len; i++) {
 		...
 		if(deletelist==""){
 			...
 			if(confirm("确定要删除？")){
 				window.location.href="<%=basePath%>company/company_doDeleteAll.action?deletelist="+deletelist+"&currentPage=<%=currentPage%>";
			}
		}
	}
</script>
	...
	<form action="<%=basePath%>company/company_doFind.action" name="companylistForm" id="companylistForm" method="post">
		<div class="place">
			...
			<ul class="placeul">
				...
				<li1><a style="color:blue;" href="<%=basePath%>company/company_doFind.action">公司列表</a></li1>
		...
		<div class="rightinfo">
			<div class="tools">
				...
				<ul class="toolbar1">
					...
					<li><span onclick="document.getElementById('companylistForm').submit();">search<img src="<%=basePath%>images/t06.png" /></span></li>
				</ul>
			</div>
			...
			<table class="tablelist">
				<thead>
					<tr>
						...
						<th>公司ID<i class="sort"><img src="<%=basePath%>images/px.gif" /></i></th>
						<th>公司名</th>
						<th>公司类型</th>
						<th>公司电话</th>
						<th>公司地址</th>
				...
				<tbody>
					<%
						...
						Company company = list.get(i); //获取到对象
					%>
					<tr>
						<td><input name="check" type="checkbox" value="<%=company.getCompanyId()%>"></input></td>
						<td><div align="center"><%=currentIndex%></div></td>
						<td><%=company.getCompanyId()%></td>
						<td onclick="javascript:location.href='<%=basePath%>company/company_doView.action?company.companyId=<%=company.getCompanyId()%>'"><%=company.getCompanyName()%></td>
						...
						<td style="width:150px;"><marquee width=150px height="28px" direction="left" behavior="scroll" scrollamount="3" onmouseover="this.stop()" onmouseout="this.start()">
								<a href=""><%=company.getCompanyDetial()%></a>
							</marquee></td>
							
						<td><a href="<%=basePath%>company/company_doView.action?company.companyId=<%=company.getCompanyId()%>" style="cursor:hand;" class="tablelink">详情</a> 
						<a href="<%=basePath%>company/company_doDelete.action?company.companyId=<%=company.getCompanyId()%>&currentPage=<%=currentPage%>" onClick="return checkDelete()" style="cursor:hand;" class="tablelink1">删除</a>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					...
					%>
					<li class="paginItem"><a href="<%=basePath%>company/company_doFind.action?currentPage=<%=firstPage-PAGE_SIZE%>&keyword=<%=keyword%>">
					...
					<li class="paginItem current"><a href="<%=basePath%>company/company_doFind.action?currentPage=<%=i%>&keyword=<%=keyword%>"><%=i%></a></li>

					<%
						continue;}
					%>

					<li class="paginItem"><a href="<%=basePath%>company/company_doFind.action?currentPage=<%=i%>&keyword=<%=keyword%>"><%=i%></a></li>
