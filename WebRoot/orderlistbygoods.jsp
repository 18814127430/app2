	...
	function deleteAll() {
		...
		for (var i = 0; i < len; i++) {
 		...
 		if(deletelist==""){
 			...
 			if(confirm("确定要删除？")){
 				window.location.href="<%=basePath%>order/order_doDeleteAll.action?deletelist="+deletelist+"&currentPage=<%=currentPage%>&goodsid=<%=goodsid%>
	";
			}
		}
	}
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<tr>
						...
						<td><%=order.getOrderSerial()%></td>
						<td><%=order.getOrderMoneyTotal()%>
							<p>
								运费：<%=order.getOrderMoneyDeliver()%></p></td>
						...
						<td><a href="<%=basePath%>order/order_doView.action?order.orderId=<%=order.getOrderId()%>" style="cursor:hand;" class="tablelink">详情</a> <a
								href="<%=basePath%>order/order_doDelete.action?order.orderId=<%=order.getOrderId()%>&goodsid=<%=goodsid%>&currentPage=<%=currentPage%>" onClick="return checkDelete()" style="cursor:hand;" class="tablelink1">删除</a></td>
					</tr>
					<%
						}
					%>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					...
					%>
					<li class="paginItem"><a href="<%=basePath%>order/order_doFindByGoodsId.action?goodsid=<%=goodsid%>&currentPage=<%=firstPage-PAGE_SIZE%>">
					...
					<li class="paginItem current"><a href="<%=basePath%>order/order_doFindByGoodsId.action?goodsid=<%=goodsid%>&currentPage=<%=i%>"><%=i%></a></li>

					<%
						continue;}
					%>

					<li class="paginItem"><a href="<%=basePath%>order/order_doFindByGoodsId.action?goodsid=<%=goodsid%>&currentPage=<%=i%>"><%=i%></a></li>
